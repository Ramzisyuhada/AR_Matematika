	...
	public class ProjectBuilder : ScriptableObject
	{
		...
		public bool BuildPlayer(bool autoRunPlayer)
		{
			...
			if (buildApplication)
			{
                ...
                BuildReport report = BuildPipeline.BuildPlayer(scenesToBuild, outputFullPath, actualBuildTarget, opt);
                ...
                {
                    UnityEngine.Debug.Log(kLogType + "BuildPlayer is finished successfully. Output: " + summary.outputPath);
                    Util.RevealOutputInFinder(outputFullPath);
                }
                else
                {
                    UnityEngine.Debug.LogError(kLogType + $"BuildPlayer failed: {summary.result}, Total errors: {summary.totalErrors}");
                    return false;
                }
            }
			return true;
